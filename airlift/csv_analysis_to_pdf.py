#UPDATES:
# 2025/02/07: Migration from JSON to Markdown
# 04/03/2025: Embedded data analysis images of in the PDF slides
# 03/11/2025: Renamed from  20250207-sim_analys_to_csv.py
# 12/16/2024: Renamed from: 20241126-analyse_csv_sim_results.py
# 11/24/2024:  LLM Perfomance seems to be good at the assessment simulation log data.  No weired LLM results noted. Recommendations are inline with intuition.
#
import os
import json
import ollama
...
# used for generating PDFs from MARP markdown

#- set target variable and load data
# TARGET_VARIABLE = 'Aircraft Quantity'
# TARGET_VARIABLE = 'wMOG capacity'
# TARGET_VARIABLE = 'Processing time'
# TARGET_VARIABLE = 'Poisson lambda'
...
# LLM_TEMPERATURE = 0.1 # 0: very conservative (good for coding and correct syntax)
LLM_TEMPERATURE = 0.1
LLM_TOP_P = 0.05
...
if TARGET_VARIABLE == 'Aircraft Quantity':
    ...
    OUTPUT_SUMMARY_TXT = 'LLM_Recomendation1_'+TARGET_VARIABLE.replace(" ", "_") + ".txt"

elif TARGET_VARIABLE == 'wMOG capacity':
    df = pd.read_csv(csv_folder+'/results-2_wMOG.csv')
    OUTPUT_SUMMARY_TXT = 'LLM_Recomendation2_'+TARGET_VARIABLE.replace(" ", "_") + ".txt"

elif TARGET_VARIABLE == 'Processing time':
    df = pd.read_csv(csv_folder+'/results-3_processing-duration.csv')
    OUTPUT_SUMMARY_TXT = 'LLM_Recomendation3_'+TARGET_VARIABLE.replace(" ", "_") + ".txt"

elif TARGET_VARIABLE == 'Poisson lambda':
    df = pd.read_csv(csv_folder+'/results-4_dynamic-route-poisson.csv')
    OUTPUT_SUMMARY_TXT = 'LLM_Recomendation4_'+TARGET_VARIABLE.replace(" ", "_") + ".txt"
    ...
    OUTPUT_SUMMARY_TXT = '/LLM_Recomendation5_'+TARGET_VARIABLE.replace(" ", "_") + ".txt"
    df = df.drop(['Min Duration', ], axis=1)        # drop the column

# identify columns with constant values where min() == max() (i.e., column with no variance)
...
# Drop column where all constant columns have the same value
...
with open(SLIDE_TYPE + "/"+OUTPUT_SUMMARY_TXT, 'w') as f:
    f.write(final_analysis)

#create pdf slides
slide_generator.save_marp_to_file(OUTPUT_SUMMARY_TXT)