...
print(f"\n LOG_TO_CONSOLE = {LOG_TO_CONSOLE}; {os.path.join(os.getcwd())}/solution/mysolution_phi4.py::MySolutionPhi4()")

class MySolutionPhi4(Solution):
    """
    ...
    def __init__(self):
        ...
        self.cost_routes_table = None           # plane type specific; flyable routes - can be flown now
    ...
    def reset(self, obs, observation_spaces=None, action_spaces=None, seed=None):
        ...
        # routes are effected by mal propery; do not recompute every time step
    ...
    def build_route_lookup_tables(self):
        '''
        Simple helper funciton that builds the look up table. Ignores route availability status (mal)
        '''
        ...
        for plane_type in self.plane_types:
            ...
            for u, v, attr in self.plane_graph[plane_type].edges(data=True):
                route_cost[(u, v)] = attr['cost']

            self.cost_routes_table[plane_type] = route_cost
    ...
    def get_path_cost(self, start: int, end: int, plane: PlaneType):
        '''
        ...
        for p in path:
            cost += self.cost_routes_table[plane][(start_location, p)]
            start_location = p
    ...
    def build_route_path_for_each_plane_type(self):
        '''
        Builds route path (subgraph) view for each plane type. Route available is when mal==0. This function needs to be recomputed every time step, but that is computationally expensive. Need better strategy.
        ...
        for plane_type in self.plane_types:
            ...
            self.view[plane_type] = nx.subgraph_view(self.plane_graph[plane_type])
    ...
    def plot_digraph(self, plane_type):
        ...
        rounded_labels = {}
        for k, v in self.cost_routes_table[plane_type].items():
    ...
    def build_global_flight_path_graph(self, state):
        ...
        self.global_view = nx.subgraph_view(self.global_flight_path_graph)
