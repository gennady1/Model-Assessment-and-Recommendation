...
# src  https://python.plainenglish.io/hungarian-algorithm-introduction-python-implementation-93e7c0890e15
...
def main():
	...
	# 			[9, 5, 6, 4, 7]])
	print("Test")
	cost_matrix = np.array([[0.0, 0.496204, 0.921583, 1.659240],
				[0.0, 0.129265, 0.136926, 0.177987],
				[99999, 99999, 99999, 99999],
				[99999, 99999, 99999, 99999]])

	# temp fix
	cost_matrix = (cost_matrix * 100).astype(int)
	print("Updated cost matrix:\n", cost_matrix)

	ans_pos = hm.hungarian_algorithm(cost_matrix.copy())#Get the element position.
	ans, ans_mat = hm.ans_calculation(cost_matrix, ans_pos)#Get the minimum or maximum value and corresponding matrix.
	...
	# ans_pos = hm.hungarian_algorithm(cost_matrix.copy())#Get the element position.
	# ans, ans_mat = hm.ans_calculation(profit_matrix, ans_pos)#Get the minimum or maximum value and corresponding matrix.
	#Show the result
	# print(f"Linear Assignment problem result: {ans:.0f}\n{ans_mat}")

if __name__ == '__main__':
	print("\n\n\tInitialized: sandbox.hungarian_alg.py")
	main()