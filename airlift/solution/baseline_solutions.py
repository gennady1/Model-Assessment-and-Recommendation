...
class ShortestPath(Solution):
    ...
    def get_active_cargo_from_state(self, state):
        ''' 
        This method returns a list of pending cargo (cargo that has not been assigned to any agent yet and is available).
        '''
    ...
    def separate_cargo_by_location_and_destination(self, cargo_list):
        '''
        This function separates the input cargo list into bins based on their location and destination.
        Each bin contains cargo items with the same location and destination.
        '''
    ...
    def select_cargo(self, cargo_bins):
        '''
        Given a dictionary of cargo bins, this method randomly selects a key (location, destination pair), and returns the corresponding list of cargo items and the selected key.
        '''
    ...
    def get_initial_agent_actions(self, obs, state, dones, pending_cargo, cargo_bins):
        '''
        This is a crucial function that determines initial actions for each agent based on the current observations, states, and availability of cargo.
        It uses helper functions like select_cargo to assign cargo to agents' flights.
        '''
    ...
    def create_path_and_update_action(self, obs, a, cargo_info, full_delivery_path, actions):
        '''
        This function creates a path for an agent to deliver the assigned cargo and updates the action dictionary accordingly.
        '''
        plane_type = obs[a]['plane_type']
    ...
    def get_full_delivery_path_by_cargo_info(self, obs, cargo_info):
        '''
        Given information about cargo items (like their location and destination),
        this method returns the full delivery path for those cargo items using a shortest path algorithm on a multiview graph.
        '''
    ...
    def check_for_valid_actions(self, obs, state, actions):
        '''
        This function checks if the current set of actions are valid.
        If not, it attempts to create new valid actions by updating the delivery path for the cargo items onboard the agent.
        ''' 
        for a in self.agents:
    ...
    # gets available edges at current time
    def filter_edge(self, u, v):
        """
        Filter DiGraph, used for Airplane Types graphs.
        These functions filter edges in a graph based on certain conditions (e.g., if an edge is currently unavailable due to maintenance).
        """
    ...
    def is_subgraph_of_multigraph(self, subgraph, multigraph):
        ...
        This function checks if one graph is a subgraph of another. It's not utilized in the provided code but can be used for assertions or testing purposes.
        """
